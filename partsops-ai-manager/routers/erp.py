...
def sync_invoice_to_erp(
    ...
    if principal.role != "admin":
        raise HTTPException(status_code=403, detail="ERP synchronization requires admin role")
    request = session.exec(select(PartRequest).where(
        PartRequest.request_id == request_id,
        PartRequest.tenant_id == tenant_id,
    )).first()
    ...
    if x_request_version and x_request_version != current_version:
        raise HTTPException(status_code=409, detail={"code": "REQUEST_VERSION_CONFLICT", "current_version": current_version})
    invoice = session.exec(select(Invoice).where(
        Invoice.request_id == request_id,
        Invoice.tenant_id == tenant_id,
    ).order_by(col(Invoice.created_at).desc())).first()
    ...
    try:
        ...
        pricing_evidence = {}
    if not isinstance(pricing_evidence, dict) or not pricing_evidence.get("approved_offer_snapshot"):
        raise HTTPException(status_code=422, detail={"code": "ERP_OFFER_SNAPSHOT_REQUIRED"})
    if request.status == RequestState.ERP_SYNCED:
        ...
        return {
            ...
            "sync": {"status": "ALREADY_SYNCED", "sync_id": sync_log.sync_id if sync_log else None, "idempotent": True},
        }
    if request.status == RequestState.ERP_SYNCING:
        raise HTTPException(status_code=409, detail={"code": "ERP_SYNC_ALREADY_RUNNING"})
    if request.status not in {RequestState.INVOICE_DRAFTED, RequestState.ERP_SYNC_FAILED}:
        raise HTTPException(status_code=409, detail={"code": "INVALID_LIFECYCLE_TRANSITION", "request_status": request.status})
    ...
    target_state = RequestState.ERP_SYNCED if result["status"] in {"SUCCESS", "ALREADY_SYNCED"} else RequestState.ERP_SYNC_FAILED
    RequestService.transition_state(
        session,
        request_id,
        tenant_id,
...
def generate_invoice(
    ...
    if principal.role not in {"finance", "admin"}:
        raise HTTPException(status_code=403, detail="Invoice creation requires finance or admin role")
