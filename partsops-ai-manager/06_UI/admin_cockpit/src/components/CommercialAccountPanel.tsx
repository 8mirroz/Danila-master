...
export function CommercialAccountPanel({ data, loading, error, onRetry, onOpenSuppliers, onCreateRequest }: CommercialAccountPanelProps) {
  if (loading) {
    return <section aria-label="Коммерческий статус" className="panel-card p-5 animate-pulse"><div className="h-4 w-40 rounded bg-surface-4" /><div className="mt-4 h-16 rounded-control bg-surface-3" /></section>;
  }

  if (error || !data) {
    return (
      <section aria-label="Коммерческий статус" className="rounded-card border border-amber-200 bg-amber-50/70 p-5 text-sm text-amber-950">
  ...
  return (
    <section aria-label="Организация и тариф" className="panel-card overflow-hidden">
      <div className="flex flex-wrap items-start justify-between gap-4 border-b border-line px-5 py-4">
        <div>
          <p className="text-[10px] font-bold uppercase tracking-[0.16em] text-ink-muted">Организация</p>
          <h3 className="mt-1 text-sm font-bold text-ink-primary">{data.organization.display_name || data.organization.organization_id}</h3>
        </div>
      ...
      <div className="grid gap-5 p-5 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.15fr)]">
        <div>
          <div className="flex items-baseline justify-between gap-3"><span className="text-xs font-semibold text-ink-secondary">{data.subscription.plan_code === 'team' ? 'Team' : data.subscription.plan_code === 'start' ? 'Start' : 'Beta trial'}</span><span data-numeric className="font-mono text-xs font-bold text-ink-primary">{data.usage.positions_used} / {data.usage.position_limit}</span></div>
          <div className="mt-2 h-2 overflow-hidden rounded-full bg-surface-3"><div className="h-full rounded-full bg-accent-primary transition-[width] duration-300" style={{ width: `${usedPercent}%` }} /></div>
          <p className="mt-2 text-[11px] text-ink-secondary">Осталось {data.usage.positions_remaining} позиций{periodEnd ? ` до ${periodEnd}` : ''}.</p>
        </div>
        <div>
          <div className="flex items-center justify-between gap-3"><p className="text-xs font-semibold text-ink-secondary">Первый полезный результат</p><span className="font-mono text-[10px] text-ink-muted">{completed.size}/{steps.length}</span></div>
          <div className="mt-2 grid gap-1.5 sm:grid-cols-2">
            {steps.map((step) => (
              <div key={step} className={`flex items-center gap-2 text-[11px] ${completed.has(step) ? 'text-emerald-700' : 'text-ink-secondary'}`}>
