...
const MatchRadarChart = ({ breakdown, score }: { breakdown?: MatchBreakdown | null; score: number }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center p-3 rounded-2xl bg-gradient-to-b from-blue-50/50 to-slate-50 border border-blue-100/60 shadow-sm relative overflow-hidden">
      ...
      <div className="grid grid-cols-3 gap-1 w-full text-[9px] text-ink-muted font-semibold text-center mt-1 border-t border-line pt-1.5">
        <div>Прайс: {price}%</div>
...
export const SupplierMatrix = ({ parts, onSelectOffer, selectedOffers = {}, requestId }: SupplierMatrixProps) => {
  ...
  return (
    <div className="bg-surface-1 border border-line rounded-2xl p-6 shadow-[0_10px_30px_rgba(37,99,235,0.05)] space-y-5 text-ink-primary">
      {/* Top Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-line-subtle pb-4 gap-3">
        ...
        <div className="flex items-center gap-2 self-start md:self-auto">
          ...
          <div className="inline-flex p-0.5 bg-surface-3/80 border border-line rounded-xl">
            <button
              onClick={() => setViewMode('direct')}
              className={`px-3 py-1 text-xs font-semibold rounded-lg transition-all ${
                viewMode === 'direct'
                  ? 'bg-surface-1 text-accent-primary shadow-sm font-bold'
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div className="p-3.5 rounded-xl bg-gradient-to-br from-blue-50/60 to-slate-50 border border-blue-100/60 flex items-center justify-between">
          <div>
            <div className="text-[10px] uppercase font-bold text-ink-muted tracking-wider">Найдено офферов</div>
            <div className="text-lg font-bold font-mono text-ink-primary mt-0.5">{metrics.total} вар.</div>
          </div>
        ...
        <div className="p-3.5 rounded-xl bg-gradient-to-br from-emerald-50/60 to-slate-50 border border-emerald-100/60 flex items-center justify-between">
          <div>
            <div className="text-[10px] uppercase font-bold text-ink-muted tracking-wider">Лучшая цена</div>
        ...
        <div className="p-3.5 rounded-xl bg-gradient-to-br from-purple-50/60 to-slate-50 border border-purple-100/60 flex items-center justify-between">
          <div>
            <div className="text-[10px] uppercase font-bold text-ink-muted tracking-wider">Макс. надежность</div>
        ...
        <div className="p-3.5 rounded-xl bg-gradient-to-br from-amber-50/60 to-slate-50 border border-amber-100/60 flex items-center justify-between">
          <div>
            <div className="text-[10px] uppercase font-bold text-ink-muted tracking-wider">Средний SLA</div>
      ...
      {viewMode === 'analogs' ? (
        ...
        <>
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 border-y border-line-subtle py-3">
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-2 whitespace-nowrap rounded-xl px-3.5 py-2 text-xs font-semibold transition-all ${
                      isSelected
                        ? 'bg-accent-primary text-white shadow-md shadow-blue-500/20 font-bold'
                        : 'bg-surface-3/80 text-ink-secondary hover:bg-surface-4/80 hover:text-ink-primary border border-line'
                    }`}
                  >
            ...
            <div className="relative min-w-[220px]">
              ...
              <input
                ...
                className="w-full pl-8 pr-3 py-1.5 text-xs bg-surface-2 border border-line rounded-xl text-ink-primary placeholder-ink-muted focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 font-medium"
              />
              {searchQuery && (
                <button
                  onClick={() => setSearchQuery('')}
                  className="absolute right-2.5 top-1/2 -translate-y-1/2 text-ink-muted hover:text-ink-secondary text-xs font-bold"
                >
                  ×
                </button>
              )}
          ...
          <div className="grid grid-cols-1 gap-5 xl:grid-cols-[minmax(0,1fr)_340px]">
            ...
            <div className="min-w-0 overflow-x-auto">
              {loading ? (
                <div className="text-xs text-ink-muted py-16 text-center flex flex-col items-center justify-center gap-2">
                  <Icon name="spinner" size={24} className="animate-spin text-accent-primary" />
                  <span className="font-semibold text-ink-secondary">Анализ цен и алгоритмов ранжирования поставщиков...</span>
                </div>
              ) : error ? (
                ...
                <div className="flex flex-col items-center justify-center gap-3 rounded-2xl border border-blue-100 bg-blue-50/40 p-8 text-center text-xs">
                  ...
                  <div>
                    <strong className="font-bold text-ink-primary text-sm block">Прямые каталожные офферы не найдены</strong>
                    <span className="text-ink-secondary text-xs mt-1 block">
                  ...
                  <div className="flex items-center gap-2 pt-2">
                    <button
                      onClick={() => setViewMode('analogs')}
                      className="px-4 py-2 bg-accent-primary text-white text-xs font-bold rounded-xl shadow-sm hover:bg-accent-strong transition-all flex items-center gap-2"
                    >
                  ...
                  {filteredMatches.map((m, idx) => {
                    ...
                    return (
                      <tr
                        ...
                        className={`border-b border-line-subtle hover:bg-blue-50/40 transition-colors cursor-pointer text-xs ${
                          isChosen
                      ...
                      >
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            <span className="font-bold text-ink-primary text-xs">{m.item.brand}</span>
                            {isBestOffer && (
                          ...
                          <div className="text-xs text-ink-secondary font-medium truncate max-w-[220px] mt-0.5">{m.item.name}</div>
                          <div className="text-[10px] text-ink-muted font-mono mt-0.5">OEM: {m.item.oem_number || '—'}</div>
                        </td>

                        <td className="px-4 py-3">
                          <div className="font-semibold text-ink-primary">{m.supplier.name}</div>
                        ...
                        <td className="px-4 py-3 font-bold text-ink-primary text-sm font-mono whitespace-nowrap">
                          {m.item.price.toLocaleString()} ₽
                        </td>

                        <td className="px-4 py-3 text-ink-secondary whitespace-nowrap">
                          <div className="font-semibold font-mono text-ink-primary">{m.item.stock_qty} шт.</div>
                          <div className="text-[10px] text-ink-muted">{m.item.delivery_days} дн. экспресс</div>
                        </td>
                        ...
                        <td className="w-[124px] px-4 py-3">
                          <ActionButton
                            ...
                            className="w-full whitespace-nowrap px-3 py-1.5 text-xs shadow-ds-sm"
                          >
            ...
            <div className="min-w-0 flex flex-col justify-between rounded-2xl border border-line bg-surface-2/50 p-4 space-y-4 shadow-sm">
              <div>
                <h4 className="text-xs font-bold text-ink-primary border-b border-line pb-2.5 mb-3 uppercase tracking-wider flex items-center justify-between">
                  <span className="flex items-center gap-1.5">
                    <Icon name="wave-square" size={15} className="text-accent-primary" />
                ...
                {selectedMatch ? (
                  <div className="space-y-3 text-xs">
                    ...
                    <div className="space-y-2 pt-1 text-ink-secondary bg-surface-1 p-3 rounded-xl border border-line/70">
                      <div className="flex justify-between items-center text-xs">
                        <span className="text-[10px] text-ink-muted uppercase font-bold">Бренд & Деталь</span>
                        <span className="font-bold text-ink-primary">{selectedMatch.item.brand}</span>
                      </div>
                      <div className="text-ink-primary font-medium text-xs leading-snug">{selectedMatch.item.name}</div>
                    ...
                    <div className="border-t border-line pt-2 space-y-1.5 bg-surface-1 p-3 rounded-xl border border-line/70">
                      <div className="flex justify-between items-center text-xs">
                        <span className="text-[10px] text-ink-muted uppercase font-bold">Поставщик</span>
                        <span className="font-semibold text-ink-primary">{selectedMatch.supplier.name}</span>
                      </div>
                      <div className="flex justify-between items-center text-xs text-ink-secondary">
                        <span className="text-[10px] text-ink-muted uppercase font-bold">Цена unit</span>
                        <span className="font-mono font-bold text-emerald-600 text-sm">
                          {selectedMatch.item.price.toLocaleString()} ₽
                        </span>
                      </div>
                      <div className="flex justify-between items-center text-xs text-ink-secondary">
                        <span className="text-[10px] text-ink-muted uppercase font-bold">Срок доставки</span>
                  ...
                  <div className="text-xs text-ink-muted text-center py-12 flex flex-col items-center justify-center gap-2">
                    <Icon name="search" size={24} className="text-ink-muted" />
              ...
              {selectedMatch && (
                <div className="pt-3 border-t border-line text-[10px] text-ink-muted font-medium flex items-center justify-between">
                  <span className="flex items-center gap-1">
                    <Icon name="user-shield" size={14} className="text-emerald-600" />
                    <span>Верифицировано AI Engine</span>
                  </span>
                  <span className="font-mono text-ink-muted">v3.0 Engine</span>
                </div>
              )}
            </div>
          </div>
