"""Scores matching candidates and emits MATCH_CANDIDATE_CREATED events."""
...
logger = logging.getLogger("automation.jobs.supplier_match")


def run(session: Session, context: AutomationContext) -> Dict[str, Any]:
    request_ids = context.payload.get("request_ids") or []
    if context.dry_run:
        return {"ok": True, "dry_run": True, "matched": 0, "skipped": len(request_ids)}
    ...
    for req_id in request_ids:
        ...
        append_request_event(
            ...
            event_type=EventType.MATCH_CANDIDATE_CREATED,
            actor_type="automation",
            actor_id=context.actor_id,
            payload={
                "candidate_count": len(context.payload.get("candidates", {}).get(req_id, [])),
                "top_score": context.payload.get("top_scores", {}).get(req_id),
            },
        )
        matched += 1
